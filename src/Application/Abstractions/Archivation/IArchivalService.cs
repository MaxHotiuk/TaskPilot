...
public interface IArchivalService
{
    ...
    Task<ArchivalJobDto> MarkBoardAsDearchivedAsync(Guid boardId, string? dearchivalReason = null, CancellationToken cancellationToken = default);
    Task ProcessArchivedBoardsAsync(CancellationToken cancellationToken = default);
    Task ProcessDearchivedBoardsAsync(CancellationToken cancellationToken = default);
    Task<bool> EnqueueArchivedBoardAsync(Guid boardId, Guid jobId, CancellationToken cancellationToken = default);
    Task<bool> EnqueueDearchivedBoardAsync(Guid boardId, Guid jobId, CancellationToken cancellationToken = default);
    Task<IEnumerable<ArchivalJobDto>> GetPendingArchivalJobsAsync(CancellationToken cancellationToken = default);
    Task<IEnumerable<ArchivalJobDto>> GetPendingDearchivalJobsAsync(CancellationToken cancellationToken = default);
